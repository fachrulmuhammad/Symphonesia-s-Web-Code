...
(function ($) {
    ...
    $.fn.tweetable = function (options) {
        ...
        var defaults = {
            ...
            username: 'symphonesia', 	//@username tweets to display
        ...
        return this.each(function (options) {
            ...
            var api = "../../../api.twitter.com/1/statuses/user_timeline.json@screen_name=";
            var count = "&count=";
                ...
                $.each(data, function (i, item) {
                    ...
                    if (defaults.replies === false) {
                        if (item.in_reply_to_status_id === null) {
                            $tweetList.append('<li class="tweet_content_' + i + '">' + item.text.replace(/#(.*?)(\s|$)/g, '<span class="hash">#$1 </span>').replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, '<a href="$&">$&</a> ').replace(/@(.*?)(\s|\(|\)|$)/g, '<a href="../../../twitter.com/$1">@$1 </a>$2')+'</li>');
                        }
                    } else {
                        $tweetList.append('<li class="tweet_content_' + i + '">' + item.text.replace(/#(.*?)(\s|$)/g, '<span class="hash">#$1 </span>').replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, '<a href="$&">$&</a> ').replace(/@(.*?)(\s|\(|\)|$)/g, '<a href="../../../twitter.com/$1">@$1 </a>$2') + '</li>');
                    }
